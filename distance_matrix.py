...
swallowVideo = 491
coughVideo = 516
vocalizationVideo = 531
# videoList = [swallowVideo, coughVideo, vocalizationVideo]

# excelName = "subjectStrainData" + str(startVideo) + "to" + str(endVideo) + ".xlsx"
# writer = pd.ExcelWriter(excelName, engine='xlsxwriter')
# writer.save()

def distance_matrix(videoInput): 
    videoNumber = str(videoInput)
    ...
    if len(indiceslist) < 3:
        textName = "notEnoughStains" + str(swallowVideo) + "to" + str(vocalizationVideo) + ".txt"
    ...
    indiceslist = [indiceslist[i][0:2] for i in range(len(indiceslist))]
    ...
    return distances, indiceslist

videosList = [(443,465,478),(566,584,598)]
# Subjects that have high-enough strains for leaned positions:
    # 4, 5

print('Processing video ' + str(19) + '...')
swallowDistances, swallowMarkers = distance_matrix(19)

swallowDistancesName = "swallowDistances" + str(19) + ".csv"
with open(swallowDistancesName, 'w') as f:
    wr = csv.writer(f)
    wr.writerows(swallowDistances)
    f.close()

swallowMarkersName = "swallowMarkers" + str(19) + ".csv"
with open(swallowMarkersName, 'w') as f:
    wr = csv.writer(f)
    wr.writerows(swallowMarkers)
    f.close()

# for i in range(len(videosList)):
#     print('Processing video ' + str(videosList[i][0]) + '...')
#     swallowDistances, swallowMarkers = distance_matrix(videosList[i][0])
#     print('Processing video ' + str(videosList[i][1]) + '...')
#     coughDistances, coughMarkers = distance_matrix(videosList[i][1])
#     print('Processing video ' + str(videosList[i][2]) + '...')
#     vocalizationDistances, vocalizationMarkers = distance_matrix(videosList[i][2])
    
#     swallowDistancesName = "swallowDistances" + str(videosList[i][0]) + ".csv"
#     with open(swallowDistancesName, 'w') as f:
#         wr = csv.writer(f)
#         wr.writerows(swallowDistances)
#         f.close()
     
#     coughDistancesName = "coughDistances" + str(videosList[i][1]) + ".csv"
#     with open(coughDistancesName, 'w') as f:
#         wr = csv.writer(f)
#         wr.writerows(coughDistances) 
#         f.close()
        
#     vocalizationDistancesName = "vocalizationDistances" + str(videosList[i][2]) + ".csv"
#     with open(vocalizationDistancesName, 'w') as f:
#         wr = csv.writer(f)
#         wr.writerows(vocalizationDistances)
#         f.close()

#     swallowMarkersName = "swallowMarkers" + str(videosList[i][0]) + ".csv"
#     with open(swallowMarkersName, 'w') as f:
#         wr = csv.writer(f)
#         wr.writerows(swallowMarkers)
#         f.close()
     
#     coughMarkersName = "coughMarkers" + str(videosList[i][1]) + ".csv"
#     with open(coughMarkersName, 'w') as f:
#         wr = csv.writer(f)
#         wr.writerows(coughMarkers)  
#         f.close()
        
#     vocalizationMarkersName = "vocalizationMarkers" + str(videosList[i][2]) + ".csv"
#     with open(vocalizationMarkersName, 'w') as f:
#         wr = csv.writer(f)
#         wr.writerows(vocalizationMarkers)
#         f.close()
